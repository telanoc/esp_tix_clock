/*
  ...
  1.4t 16 Sep 2015 - Modified by Pete Cervasio for TIX clock project
                     (examples not changed though)
*/
...
time_t now() {
  ...
  if (nextSyncTime <= sysTime) {
    if (getTimePtr != 0) {
      unsigned long msec;
      time_t t = getTimePtr(&msec);
      if (t != 0) {
        setTime(t);
		/* adjust milliseconds for more accuracy */
        prevMillis -= msec;
      } else {
