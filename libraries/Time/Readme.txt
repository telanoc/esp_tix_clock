...
Note: Modified by Pete Cervasio for his Tix clock in order to get slightly
more accurate time setting.  The time sync callback still returns a time_t
value, but it is called with a pointer to an unsigned long which will be
written with the number of milliseconds in the NTP time packet.  The library
code then uses that number to adjust the prevMillis value.  The result is
pretty accurate time setting (modulo the accuracy of the server being queried,
along with network lag).  The NTP example was modified so it should compile
but I haven't run it.  The code I use is in the esp_tix_clock sketch.

I don't recall changing anything else, though I may have run Time.* through
'indent'.
